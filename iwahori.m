...
/* 
================================================================================
PREAMBLE: Loading the packages necessary for this file.  You might need to 
change the file path, depending on where 'hecke_matrices.m' lives in your 
directory. 
================================================================================
*/
...
GOAL:
Computes the action of Hecke operators E_\ell and F_\ell on H_3(I(3, p), C). 
================================================================================
*/
function IwahoriHecke(p, l_list : toggle := 1)
    iwahoriIndex := (1+p) * (1+p+p^2);
    M_Kernel := IwahoriMatrix(p);
    ...
    FDict := AssociativeArray();

    for l in l_list do
        E_Unis, F_Unis := BuildHeckeMatrices(l);
        P<T> := PolynomialRing(Rationals());
// ----------------------------------------------------------------------------
// Computes the characteristic polynomial of E_\ell and F_\ell on 
// H_3(I(3, p), C).  This is redundant since they are conjugate. 
// ----------------------------------------------------------------------------
        if toggle eq 0 then
            runningSumE := ZeroMatrix(Integers(), NumberOfRows(kernelBasis), NumberOfColumns(kernelBasis));
            runningSumF := ZeroMatrix(Integers(), NumberOfRows(kernelBasis), NumberOfColumns(kernelBasis));
            ...
            Char_Poly_E := P!CharacteristicPolynomial(E);
            EDict[l] := Factorization(Char_Poly_E);
            "The factored characteristic polynomial of T(l, 1) is...";
            Factorization(Char_Poly_E);
            
            "Now computing the action of Hecke operator T(l, 2) for l = " cat IntegerToString(l) cat "...";
            for i in [1..#F_Unis] do
                tempF := Matrix(Integers(), F_Unis[i]);
                row_indicies := [0 : k in [1..iwahoriIndex]]; 
                for j in [1..iwahoriIndex] do
                    target := tempF * A[j];
                    key := CanonicalKey(target, p);
                ...
                tempRes := Matrix([ kernelBasis[indx] : indx in row_indicies ]);
                runningSumF := runningSumF + tempRes;
            end for;

            F := Solution(Transpose(kernelBasis), Transpose(runningSumF));
            Char_Poly_F := P!CharacteristicPolynomial(F);
            FDict[l] := Factorization(Char_Poly_F);
            "The factored characteristic polynomial of T(l, 2) is...";
            Factorization(Char_Poly_F);
...
// ----------------------------------------------------------------------------
// Computes the characteristic polynomial of F_\ell on H_3(I(3, p), C).
// ----------------------------------------------------------------------------
        else 
            runningSumF := ZeroMatrix(Integers(), NumberOfRows(kernelBasis), NumberOfColumns(kernelBasis));
